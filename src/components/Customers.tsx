...
export default function Customers({ customers, setCustomers }: CustomersProps) {
  ...
  return (
    <div className="space-y-6 font-sans text-slate-800 animate-in fade-in duration-300 pb-12 relative print:m-0 print:p-0">
      ...
      <div className="print:hidden">
        <div className="flex justify-between items-center mb-2">
          ...
          <button 
            ...
            className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-full text-sm font-semibold transition shadow-md cursor-pointer"
          >
            Add Customer
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col">
          <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <h3 className="font-bold text-slate-800 text-lg">All Customers</h3>
              <span className="px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-600 text-xs font-bold">{customers.length} Total</span>
            </div>
              ...
              <tbody className="divide-y divide-slate-50">
                {customers.map((c, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="py-4 px-6 text-center font-semibold text-slate-500">{idx + 1}</td>
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        {c.photo ? (
                          <img src={c.photo} alt={c.name} className="w-10 h-10 rounded-full object-cover shrink-0 border-2 border-indigo-100" />
                        ) : (
                    ...
                    <td className="py-4 px-6 text-center">
                      <span className="inline-flex items-center justify-center min-w-[24px] h-6 px-2 rounded bg-rose-50 text-rose-600 font-bold text-xs border border-rose-100">
                        {c.sales}
                      </span>
                    </td>
                    <td className="py-4 px-6">
          ...
          <div className="p-4 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500 bg-slate-50/30">
            ...
            <div className="flex gap-1">
              <button className="px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-white font-medium bg-slate-50 cursor-pointer">Previous</button>
              <button className="px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-white font-medium bg-slate-50 cursor-pointer">Next</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in print:bg-white print:p-0 print:absolute print:inset-0 print:block">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200 print:shadow-none print:max-w-full print:h-full print:overflow-visible">
            ...
            <div className="sticky top-0 bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between z-10 print:hidden">
              <h2 className="text-xl font-bold text-slate-800">গ্রাহকের তথ্য</h2>
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setViewCustomer(null)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-full transition cursor-pointer">
                  <X size={20} />
                </button>
              </div>
            </div>
            ...
            <div className="hidden print:block text-center border-b-2 border-indigo-600 pb-4 mb-6 pt-8">
              ...
              <h2 className="text-xl font-bold text-slate-800 mt-2">গ্রাহকের প্রোফাইল</h2>
            </div>
            ...
            <div className="p-6 print:p-0 print:px-8">
              <div className="flex flex-col md:flex-row gap-8">
                ...
                <div className="w-full md:w-48 flex flex-col items-center gap-4 shrink-0">
                  <div className="w-32 h-32 rounded-xl bg-slate-100 border-2 border-slate-200 overflow-hidden flex items-center justify-center print:border-indigo-200">
                    {viewCustomer.photo ? (
                ...
                <div className="flex-1">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
                    <div>
                      ...
                      <p className="text-lg font-bold text-slate-800">{viewCustomer.name}</p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-slate-400 mb-1 uppercase tracking-wider">আইডি নং (Customer ID)</p>
                      <p className="text-base font-bold text-indigo-600 font-mono">{viewCustomer.id}</p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-slate-400 mb-1 uppercase tracking-wider">পিতার নাম</p>
                      <p className="text-base font-medium text-slate-700">{viewCustomer.fatherName || 'প্রদান করা হয়নি'}</p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-slate-400 mb-1 uppercase tracking-wider">মাতার নাম</p>
                      <p className="text-base font-medium text-slate-700">{viewCustomer.motherName || 'প্রদান করা হয়নি'}</p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-slate-400 mb-1 uppercase tracking-wider">জন্ম তারিখ</p>
                      <p className="text-base font-medium text-slate-700">{viewCustomer.dob || 'প্রদান করা হয়নি'}</p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-slate-400 mb-1 uppercase tracking-wider">এনআইডি/পাসপোর্ট নং</p>
                      <p className="text-base font-medium text-slate-700">{viewCustomer.idNumber || 'প্রদান করা হয়নি'}</p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-slate-400 mb-1 uppercase tracking-wider">মোবাইল নাম্বার</p>
                      <p className="text-base font-medium text-slate-700">{viewCustomer.phone}</p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-slate-400 mb-1 uppercase tracking-wider">ইমেইল</p>
                      <p className="text-base font-medium text-slate-700">{viewCustomer.email}</p>
                    </div>
                    <div className="sm:col-span-2">
                      <p className="text-xs font-semibold text-slate-400 mb-1 uppercase tracking-wider">বর্তমান/স্থায়ী ঠিকানা</p>
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 print:border-indigo-100">
                <h3 className="text-sm font-bold text-slate-800 mb-4 uppercase tracking-wider">লেনদেনের সারসংক্ষেপ</h3>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-rose-50/50 border border-rose-100 rounded-xl p-4 flex items-center justify-between print:border-rose-300">
                    <div>
                      <p className="text-xs font-bold text-rose-600 mb-1">মোট জমি বিক্রয়</p>
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 print:hidden">
                <h3 className="text-sm font-bold text-slate-800 mb-4 uppercase tracking-wider">সংযুক্ত ডকুমেন্টস (Attachments)</h3>
                <div className="flex gap-4">
                  <div className="border border-slate-200 rounded-xl p-3 w-48 hover:shadow-md transition group">
                    ...
                    <div className="flex gap-2">
                      <button className="flex-1 flex items-center justify-center gap-1 py-1.5 bg-indigo-50 text-indigo-600 rounded text-[10px] font-bold hover:bg-indigo-600 hover:text-white transition">
                        <Eye size={12} /> View
                      </button>
                      <button className="flex-1 flex items-center justify-center gap-1 py-1.5 bg-slate-50 text-slate-600 rounded text-[10px] font-bold hover:bg-slate-200 transition">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in print:hidden">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200">
            <div className="sticky top-0 bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between z-10">
              ...
              <button onClick={() => setIsModalOpen(false)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-full transition cursor-pointer">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="flex-1 space-y-5">
                  <div>
                    ...
                    <input required type="text" name="name" value={formData.name} onChange={handleInputChange} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition" placeholder="গ্রাহকের সম্পূর্ণ নাম লিখুন" />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-sm font-semibold text-slate-700 mb-1.5">পিতার নাম</label>
                      <input type="text" name="fatherName" value={formData.fatherName} onChange={handleInputChange} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition" placeholder="পিতার নাম লিখুন" />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-slate-700 mb-1.5">মাতার নাম</label>
                      <input type="text" name="motherName" value={formData.motherName} onChange={handleInputChange} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition" placeholder="মাতার নাম লিখুন" />
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-sm font-semibold text-slate-700 mb-1.5">জন্ম তারিখ</label>
                      <input type="date" name="dob" value={formData.dob} onChange={handleInputChange} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition" />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-slate-700 mb-1.5">আইডি নং (NID/Passport)</label>
                      <input type="text" name="idNumber" value={formData.idNumber} onChange={handleInputChange} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition" placeholder="আইডি কার্ড নাম্বার" />
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div className="sm:col-span-2">
                      <label className="block text-sm font-semibold text-slate-700 mb-1.5">ঠিকানা</label>
                      <textarea name="address" value={formData.address} onChange={handleInputChange} rows={2} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition resize-none" placeholder="বর্তমান বা স্থায়ী ঠিকানা" />
                    </div>
                    <div className="sm:col-span-2">
                      <label className="block text-sm font-semibold text-slate-700 mb-1.5">মোবাইল নাম্বার <span className="text-rose-500">*</span></label>
                      <input required type="tel" name="phone" value={formData.phone} onChange={handleInputChange} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition" placeholder="01XXX-XXXXXX" />
                    </div>
                  </div>
                </div>
                ...
                <div className="w-full lg:w-72 flex flex-col gap-6">
                  ...
                  <div>
                    ...
                    <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-slate-300 bg-slate-50 hover:bg-indigo-50 hover:border-indigo-300 transition rounded-xl cursor-pointer group overflow-hidden relative">
                      {photoPreview ? (
                        <img src={photoPreview} alt="Preview" className="w-full h-full object-contain p-2" />
                      ) : (
                        <>
                          <div className="w-12 h-12 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center mb-2 group-hover:scale-110 transition-transform">
                            <ImageIcon size={20} />
                          </div>
                          <span className="text-xs font-semibold text-indigo-600">ছবি আপলোড করুন</span>
                          <span className="text-[10px] text-slate-400 mt-1">PNG, JPG (Max 2MB)</span>
                        </>
                      )}
                  ...
                  <div>
                    ...
                    <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-300 bg-slate-50 hover:bg-emerald-50 hover:border-emerald-300 transition rounded-xl cursor-pointer group text-center px-4">
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 flex justify-end gap-3">
                <button 
                  ...
                  className="px-6 py-2.5 rounded-xl border border-slate-200 text-slate-600 font-bold text-sm hover:bg-slate-50 transition cursor-pointer"
                >
                  বাতিল করুন
                </button>
                <button 
                  type="submit" 
                  className="px-8 py-2.5 rounded-xl bg-indigo-600 text-white font-bold text-sm hover:bg-indigo-700 shadow-md shadow-indigo-600/20 transition cursor-pointer"
                >
                  যুক্ত করুন
                </button>
              </div>
